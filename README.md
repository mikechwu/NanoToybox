...
- **Branded link previews** — every `/c/:code` share link unfurls in Slack, X, Facebook, LinkedIn, etc. as a 1200×630 image card. At publish time, `preparePublishRecord` builds a canonical PCA camera, projects the first dense frame of the capsule, and pre-bakes two artifacts into the D1 `preview_scene_v1` column: a poster scene (≤32 atoms + bonds) for the OG card, and an embedded `thumb` payload (derived from the selected preview subject — the largest bonded cluster when a dominance guard passes, otherwise the full frame; see ADR D138) used to draw the compact 96 px thumbnail next to each row in the signed-in `/account/` uploads list. The poster route lazily backfills any null scene and serves with an ETag bound to the scene hash.
...
- **Quick Share (anonymous 72-hour links)** — alongside the authenticated publish flow, a signed-out visitor can mint a Turnstile-gated share link with no account, served through `/api/capsules/guest-publish` and auto-expired by a cron sweep after 72 hours. Rollout is gated by the `GUEST_PUBLISH_ENABLED` flag (default off); when on, the signed-out share panel surfaces a Quick Share primary card with OAuth (Google / GitHub) as the secondary option.
