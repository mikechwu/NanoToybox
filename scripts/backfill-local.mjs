 ...
 * preview scene, and writes it back to `preview_scene_v1`.
 *
...
const { projectCapsuleToSceneJson } = await import(path.join(ROOT, 'src/share/publish-core.ts'));
const { validateCapsuleFile } = await import(path.join(ROOT, 'src/history/history-file-v1.ts'));
const { CURRENT_THUMB_REV } = await import(path.join(ROOT, 'src/share/capsule-preview-scene-store.ts'));
