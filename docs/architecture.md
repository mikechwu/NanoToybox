...
- **WatchCinematicCameraToggle** — compact pill positioned between the info panel and bonded-clusters panel. Label "Cinematic Camera", status text driven by the `cinematicCameraStatus` enum via a `STATUS_TEXT` record (no boolean re-derivation). Uses `aria-pressed` toggle. Props: `enabled`, `active`, `status: SnapshotCinematicCameraStatus`, `onToggle`.
