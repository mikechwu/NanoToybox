...
**Camera apply contract:** `renderer.getOrbitCameraSnapshot()` reads the live OrbitControls pose (watch/js/watch-renderer.ts + lab/js/renderer.ts). `renderer.applyOrbitCameraSnapshot(snapshot)` on the Lab renderer cancels any in-flight camera animation, sets camera position + target + up, updates the projection matrix if `fovDeg` changed, performs a silent controls update, and recomputes focus distance. When the seed carries `camera === null` (legacy token), the hydrate falls back to `renderer.fitCamera()`.
...
**Worker velocity delivery (`src/types/worker-protocol.ts`, `lab/js/simulation-worker.ts`, `lab/js/worker-bridge.ts`, `lab/js/runtime/worker-lifecycle.ts`):** the `appendMolecule` command gained an optional `velocities: Float64Array` (interleaved `vx,vy,vz`). The worker handler writes `engine.vel.set(velocities, atomOffset*3)` **before** `sceneVersion++` so the first post-append snapshot carries real momentum. Previously the snapshot reconciler zeroed main-thread velocities on the first `frameResult`; hydrate now delivers motion state end-to-end instead of relying on main-thread-only writes. Velocity-length mismatches warn instead of silently clipping.
