...
Shared cross-app boundary for the Continue funnel (the Watch→Lab "Continue" action that routes the current frame into a seeded Lab tab). Both apps import from this folder: Watch uses the codec + storage prefix to write; Lab uses the same codec and the normalizer to read. Keeping the schema here (not under either app) guarantees the two sides cannot drift.
...
User-visible flow: in Watch, the Continue half of the `WatchLabEntryControl` capsule opens a new Lab tab at `/lab/?from=watch&handoff=<token>`. Lab boots, recognizes the URL flag, skips its default C60 auto-load, consumes the handoff from `localStorage`, and runs a transactional hydrate. The hydrated scene itself is the arrival acknowledgement — there is no arrival provenance pill, banner, or any other arrival-UI surface. Hydrate success is confirmed by the rendered scene plus a `[lab.boot] watch handoff hydrated` console.info trace (extended with `velocitySource`, `unresolvedVelocityFraction`, `colorAssignmentCount`, `hasCamera`).

Canonical terminology: the Watch-side primary button is **Continue** (previously called "Remix" / "From this frame"); the localStorage-backed transport is the **handoff**; the payload inside a handoff token is the **seed** (`WatchLabSceneSeed`); per-atom velocity state is referred to as **motion state** (per-atom mass is not tracked on the wire).
