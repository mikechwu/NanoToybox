...
│   │   │   ├── onboarding.ts        # Coachmark scheduling + page-load onboarding overlay gate (isOnboardingEligible, subscribeOnboardingReadiness, markOnboardingDismissed / wasOnboardingDismissedInSession). Welcome card + atom-interaction hint ("Drag any atom to start", no tail, convex-hull target pick) suppressed on Watch→Lab handoff boots via the shared `isWatchHandoffBoot()` predicate (see src/watch-lab-handoff/watch-handoff-url.ts) + `atomdojo.onboardingDismissed` sessionStorage sentinel.
...
│   │   │   ├── bonded-group-appearance-runtime.ts # Stable-ID projection model: atomIds canonical for rendering+export. writeAssignments/syncToRenderer/pruneAndSync project from atomIds. Group-to-atom color translation + renderer sync (annotation model). `snapshotAssignments()` + `restoreAssignments(prev)` provide REPLACE-semantics capture/install (structural deep-copy; not additive) used by the Watch→Lab hydrate transaction — passing `[]` wipes all prior assignments
...
│   │   │   ├── timeline-atom-identity.ts    # Stable atom ID tracker for export (append, compaction, capture lifecycle; snapshot/restore for transactional rollback)
│   │   │   ├── atom-metadata-registry.ts    # Persistent atom metadata keyed by stable atom ID (element, source; snapshot/restore for transactional rollback)
...
│   │   │   ├── watch-handoff.ts             # Consumes Watch→Lab handoff from URL + localStorage; returns `ConsumeOutcome` ('none' | 'ready' | 'rejected'); always scrubs URL
│   │   │   └── hydrate-from-watch-seed.ts   # Transactional hydrate across 5 authorities (physics, worker, renderer, scene-state, tracker+registry) using appendMolecule+clearScene primitives; rollback via snapshots (including camera + color); hydration-lock coordinated with frame-runtime
│   │   ├── scene.ts              # Scene commit/clear/load (transaction-safe); exports SceneState type + cloneSceneState / restoreSceneStateInPlace for hydrate rollback
...
│   │   ├── physics.ts            # Tersoff force engine + interaction forces; updateBondList() delegates to buildBondTopologyAccelerated, bonds typed as BondTuple[]; `appendMolecule` + `clearScene` primitives (canonical hydrate building blocks — restoreCheckpoint is for rollback only). `appendMolecule` accepts optional per-atom `velocities` (interleaved vx,vy,vz; Float64Array) so the worker's first post-append snapshot carries real momentum instead of zero-initialized velocities. `_ensureNeighborListWorkspace()` shared by init + restoreCheckpoint; PhysicsConfig.dampingRefDurationFs threaded through three-arg setTimeConfig
...
│   │   ├── worker-bridge.ts      # Main↔Worker bridge protocol; proactively clears `latestSnapshot` at mutation start (restoreState/appendMolecule) and when an ack advances `sceneVersion`
...
│   ├── watch-lab-handoff/            # Shared Watch→Lab handoff schema + normalization (consumed by both apps)
│   │   ├── watch-handoff-url.ts      # Shared `isWatchHandoffBoot()` predicate — single source of truth for both the Lab boot gate (`_hasPendingWatchHandoff`) and the onboarding gate (`isOnboardingEligible`). Detects the `?from=watch&handoff=<token>` URL shape before consume-path URL scrubbing.
│   │   ├── watch-lab-handoff-shared.ts # Schema + base64 codec, `HANDOFF_STORAGE_PREFIX`, `deserializeAndValidate`, `ConsumeReason` union. `WatchLabSceneSeed` fields: atoms, velocities, bonds, boundary, workerConfig, `colorAssignments` (stable-atomId color records), `camera` (orbit pose snapshot or null), refined `provenance` ({ historyKind, velocitySource, velocitiesAreApproximated, unresolvedVelocityFraction }). Legacy tokens without camera / colorAssignments / refined-provenance remain accepted (new fields optional at validate time).
│   │   └── normalize-watch-seed.ts   # Single-pass normalization: WatchLabSceneSeed → NormalizedWatchHydratePayload (localStructureAtoms, velocities, bonds, boundary, workerConfig, colorAssignments, camera, refined provenance). Legacy-token defaults: camera → null, colorAssignments → [], velocitySource derived from velocitiesAreApproximated, unresolvedVelocityFraction → 0.
...
│       ├── use-timed-cue.ts              # Shared React hook: useTimedCue({ triggerToken, durationMs }) → { active, animKey }. Baseline-on-first-observation, duration timer, animKey for re-key-on-firing. Used by WatchLabEntryControl's primary-tooltip auto-cue.
...
│       ├── watch-controller.ts       # Non-React facade: orchestrates domain services, owns RAF clock, snapshot publication, transactional file open, interpolation runtime lifecycle; exposes buildCurrentFrameLabHref(), openLabFromCurrentFrame(), notifyLabMenuClosed(). `_currentFrameHrefCache` identity tuple is 5 components: `(documentKey, displayFrameKey, topologyFrameKey, restartFrameId, cameraIdentity)` where `cameraIdentity` is a quantized `(position, target, fovDeg)` string (POSITION_Q=0.01 Å, FOV_Q=0.5°; `up` omitted). `buildCurrentFrameLabHref` passes color + camera getters into the seed builder; click path (`openLabFromCurrentFrame` → `projectCurrentFrameHref`) re-reads the live camera and requires ALL 5 components to match before reusing — a miss purges the stale token via `removeWatchToLabHandoff` and mints a fresh one.
...
│       ├── watch-lab-seed.ts         # `canBuildWatchLabSceneSeed` predicate + `buildWatchLabSceneSeed` extractor (pulls atoms/velocities/bonds/boundary/workerConfig from the current playback frame). `BuildArgs` gained optional `getColorAssignments` + `getOrbitCameraSnapshot`. Per-atom velocity source tracking collapses to a single tag (`restart` / `central-difference` / `forward-difference` / `backward-difference` / `mixed` / `none`); `unresolvedVelocityFraction` computed pre-null-promotion. Unknown atomIds in color assignments are dropped with a console.warn at build time.
...
│       ├── watch-playback-model.ts   # WatchTopologySource abstraction + LoadedWatchHistory discriminated union (full | capsule); getInteractionAtTime(timePs) method; separated sampling channels, bidirectional playback, speed 0.5x–20x, repeat; handoff helpers: getDisplayFrameIndexAtTime, getTopologyFrameIdAtTime, canApproximateVelocityAtDisplayFrame, getNearestRestartFrameIdAtTime
...
│       ├── hooks/
│       │   └── use-timeline-milestone-tokens.ts # `useTimelineMilestoneTokens(snapshot) → number`: encodes the once-per-file + arm-then-fire + paused-seek-coalesce rules for the 50% / 100% timeline auto-cue. Resets fired-state on `fileIdentity` change; requires each milestone to be observed with `currentTimePs < threshold` at least once before it can fire (so 80% share-code entry does not flash both cues at mount); coalesces to at most one milestone per effect run with end-first priority (scrub 10%→95% while paused fires end only).
│       └── components/
...
│           ├── WatchTopBar.tsx           # Compact inline file-identity bar (`.watch-topbar`) in bottom-chrome toolbar: kind chip + short display name + link/file icon buttons
│           ├── WatchCinematicCameraToggle.tsx # "Cinematic Camera" label + CSS toggle switch in bottom-chrome toolbar
...
│           ├── WatchLabEntryControl.tsx  # Primary pill ("Interact From Here") + caret-toggled disclosure popover (secondary: "Open a Fresh Lab"). Primary routes the current frame to a seeded Lab tab; caret uses `aria-haspopup="true"` and the popover has `role="group"` (disclosure semantics, NOT menu / menuitem). Primary-tooltip + menu are mutually exclusive via `data-menu-open` on `.watch-lab-entry` with CSS `:has(...)` suppression. Menu always opens above. Primary tooltip also fires as a 5s auto-cue (1s fade-in / 3s hold / 1s fade-out) on 50% and 100% timeline milestones (once-per-file, arm-then-fire, paused-seek coalescing) via `useTimedCue(useTimelineMilestoneTokens(snapshot))`. Hidden `aria-live="polite"` sibling announces the cue.
...
#### `src/watch-lab-handoff/` — Watch→Lab handoff schema and seed normalization

Shared cross-app boundary for the Watch→Lab handoff funnel (the "Interact From Here" action that routes the current frame into a seeded Lab tab). Both apps import from this folder: Watch uses the codec + storage prefix to write; Lab uses the same codec and the normalizer to read. The folder also hosts `watch-handoff-url.ts`, whose `isWatchHandoffBoot()` predicate is the single source of truth for both the Lab boot gate and the onboarding-suppression gate. Keeping the schema here (not under either app) guarantees the two sides cannot drift.

- **`src/watch-lab-handoff/watch-lab-handoff-shared.ts`** — wire-format single source of truth. Owns: `WatchLabSceneSeed` schema, base64 codec, `HANDOFF_STORAGE_PREFIX` for the `localStorage` key namespace, `deserializeAndValidate()` (returns a discriminated result — seed or structured reject reason), and the `ConsumeReason` union used by Lab's consume outcome. Seed carries: atoms + velocities + bonds + boundary + `workerConfig`, plus `colorAssignments: WatchLabColorAssignment[]` (stable-id quartet `{ id, atomIds[], colorHex, sourceGroupId }` — no dense indices on the wire), `camera: WatchLabOrbitCamera | null` (`{ position, target, up, fovDeg }` — exact orbit pose), and refined `provenance: { historyKind, velocitySource, velocitiesAreApproximated, unresolvedVelocityFraction }`. `WatchLabVelocitySource` = `'restart' | 'central-difference' | 'forward-difference' | 'backward-difference' | 'mixed' | 'none'`. Helpers `isValidCamera` and `isValidColorAssignment` compose into `isValidSeed` / `isValidPayload`; new fields are optional at validate time so legacy tokens (pre-camera, pre-colorAssignments, 2-field provenance) still parse. No framework deps.

- **`src/watch-lab-handoff/normalize-watch-seed.ts`** — single-pass normalization from `WatchLabSceneSeed` (wire shape) to `NormalizedWatchHydratePayload` (internal shape consumed by `lab/js/runtime/handoff/hydrate-from-watch-seed.ts`). Fields: `localStructureAtoms`, `velocities`, `bonds`, `boundary`, `workerConfig`, `colorAssignments`, `camera`, `provenance` (refined shape). Legacy-token defaults: `camera` → null, `colorAssignments` → [], `velocitySource` derived from `velocitiesAreApproximated` (true → `'mixed'`, false → `'restart'`), `unresolvedVelocityFraction` → 0. Defensive `VALID_VELOCITY_SOURCES` coercion covers direct-call paths that bypass the validator; camera copy is array-guarded. Pure function, no side effects, no framework deps.

`src/types/worker-protocol.ts` was extended with an optional `PhysicsConfig.dampingRefDurationFs` so the handoff payload can carry the exact damping reference duration through the worker boundary, and the `appendMolecule` worker command gained an optional `velocities: Float64Array` field (interleaved vx,vy,vz). The worker handler writes `engine.vel.set(velocities, atomOffset*3)` **before** incrementing `sceneVersion`, so the first post-append snapshot carries real momentum instead of being zeroed by the snapshot reconciler on the first frameResult. Length mismatches produce a warn rather than a silent clip. When absent, the worker keeps its previous default (zero-initialized velocities).
...
- `timeline-transfer-dialog.tsx` — unified Transfer dialog hosting Download and Share tabs; five Share-panel states (success / loading / unverified / signed-out / signed-in) plus popup-blocked sub-panel and `ShareActions` sub-component. Exports `TRANSFER_HINT_COPY = "Share & Download"` — the trigger-button label / tooltip copy (NOT "Transfer history"). The trigger also renders a 5-second 1-3-1 discoverability cue (1s fade-in, 3s hold, 1s fade-out) 5s after the first atom interaction (`useAppStore.hasAtomInteraction`); fires once per page load and is device-agnostic (bypasses `@media (hover: none)` via `.timeline-hint--force-visible`). `TransferTrigger` intentionally does NOT consume `useTimedCue` because its trigger shape (boolean event + 5s-delay-then-show) differs from Watch's (token-change + immediate-show).
...
- **Active (live/review):** Scrub-interactive track with fill+thumb, `ClearTrigger` (ActionHint-wrapped) in action slot, "Restart here" overlay anchor in review mode (positioned at restart target progress; NOT ActionHint-wrapped — the wrapping hover-tooltip was removed as a duplicate of the pill's own affordance). The restart affordance uses a two-element DOM contract: `<span class="timeline-restart-anchor">` owns positioning / centering (horizontal transform) and wraps `<button class="timeline-restart-button">` (accent-filled pill with circular-arrow icon, "Restart here" label, and a CSS `clip-path` skewed-triangle downward pointer). Separating centering transform from the button's hover transform is the point — regression-locked by `tests/unit/timeline-bar-lifecycle.test.tsx`. Pointer geometry (clip-path polygon) supports **skewed triangles**: the base stays on the pill's straight-bottom segment while the tip tracks the marker via a `--tail-skew` CSS var; all numbers derive from measured pill dims + CSS-var pointer base width (no hard-coded pixel constants in the TS clamp). Mode-switch segments (Simulation, Review) are each ActionHint-wrapped.
...
- **scene-runtime.ts** — scene mutation wrappers, scene-to-store projection, worker scene mirroring. Native `commitMolecule` now **awaits** `worker.appendMolecule` ack (no more fire-and-forget) so main↔worker scene versions never diverge mid-hydrate. Also hosts `hydrateFromWatchSeed`, a thin wrapper over `runtime/hydrate-from-watch-seed.ts` that runs under the same scene-mutation serialization.
- **worker-lifecycle.ts** — worker bridge creation, init, stall detection (5s warning / 15s fatal), teardown. `restoreState` returns `Promise<{ok}>` (was `Promise<void>`); init no longer gated on `atoms.length > 0` (a zero-atom init is a valid initial state for a pending hydrate). On `ok: false`, the worker's `onFailure` hook would normally clobber main-thread state with a restart; the hydrate path gates that recovery via `_hydrationActive` in `main.ts` so rollback owns state (see [Watch→Lab Handoff](#watchlab-handoff-funnel)).
...
- **onboarding.ts** — coachmark scheduling + page-load onboarding overlay gate (`isOnboardingEligible`, `subscribeOnboardingReadiness`, `markOnboardingDismissed()` / `wasOnboardingDismissedInSession()` — sessionStorage key `atomdojo.onboardingDismissed`). The welcome card ("Atom Simulation Studio") is suppressed on Watch→Lab handoff boots: `lab/js/main.ts` runs `if (isWatchHandoffBoot()) markOnboardingDismissed();` at module load (writing the sentinel **before** the consume path scrubs the URL), using the shared `isWatchHandoffBoot()` predicate from `src/watch-lab-handoff/watch-handoff-url.ts` — the same predicate that feeds the boot gate `_hasPendingWatchHandoff`. The Lab atom-interaction hint (floating bubble "Drag any atom to start"; no tail; target atom chosen via 2D convex-hull centermost pick; dismisses on first atom interaction via `markAtomInteractionStarted`) is gated by the same predicate plus the `atomdojo.onboardingDismissed` sentinel.
...
- **watch-handoff.ts** — `consumeWatchToLabHandoffFromLocation()` inspects `window.location` and `localStorage`, returning a `ConsumeOutcome` discriminated union (`'none'` — no flag; `'ready'` — validated seed to hydrate; `'rejected'` — flag present but seed missing / expired / malformed, with a structured reason). Always scrubs the handoff query params from the URL before returning so Lab reload never re-enters the hydrate path. Reads the shared schema + codec from `src/watch-lab-handoff/watch-lab-handoff-shared.ts`.
- **hydrate-from-watch-seed.ts** — transactional commit across the 5 authorities (main physics, worker, renderer, scene-state, tracker + metadata registry) **plus** camera and color. Built exclusively from the canonical hydrate primitives: `physics.appendMolecule` + `physics.clearScene`, `worker.clearScene` + `worker.appendMolecule` (the same primitives native `commitMolecule` uses — `restoreCheckpoint` is for rollback only). `appendMolecule` on both sides now carries the seed's per-atom `velocities` so momentum survives the worker handoff. Opens the hydration lock via the `setHydrationActive?` dep on entry and releases it on commit or rollback. Rollback uses the `SceneState` clone from `scene.ts` (`cloneSceneState` / `restoreSceneStateInPlace`) plus `snapshot()` / `restore()` on the tracker and metadata registry, and additionally restores both camera pose (`renderer.applyOrbitCameraSnapshot(capture.camera)`) and color assignments (`appearance.restoreAssignments(capture.colorAssignments)` — REPLACE semantics). Rollback sub-failures accumulate into `cause: { originatingCause, rollbackSubFailures }` instead of being swallowed. On success, camera is applied after the renderer rebuild step (falling back to `renderer.fitCamera()` when `camera === null`), and color is applied unconditionally via `restoreAssignments` so an empty rebuilt list wipes prior Lab color state. Uses `normalize-watch-seed.ts` to reduce the wire payload to a single internal shape before any authority is touched.

**Primary user-facing surfaces** (in the React tree): DockLayout, DockBar, SettingsSheet, StructureChooser, SheetOverlay, StatusBar, FPSDisplay, AccountControl, TopRightControls, CameraControls, OnboardingOverlay, BondedGroupsPanel, TimelineBar. **Supporting subcomponents** (composed by primary surfaces): Segmented, Icons, ActionHint. **Hint infrastructure:** ActionHint wraps 4 timeline controls (Start Recording, Simulation segment, Review segment, ClearTrigger) plus ReviewLockedControl and Segmented; the "Restart here" pill is NOT ActionHint-wrapped (its own label + accent fill convey intent; a hover tooltip would have been a duplicate hint); desktop/keyboard only (touch hidden via CSS media query). timeline-hints.ts is the single source of truth for all timeline tooltip copy (`TIMELINE_HINTS` constant). **Timeline helper modules** (composed by TimelineBar): timeline-format.ts (time formatting + progress), timeline-mode-switch.tsx (mode rail widget), timeline-clear-dialog.tsx (clear confirmation dialog + trigger), timeline-export-dialog.tsx (Download-tab artifact UI), timeline-transfer-dialog.tsx (unified Download/Share dialog), timeline-after-paint.ts (paint-deferred scheduler for INP-sensitive work), timeline-performance.ts (User Timing `measureSync` wrapper), timeline-hints.ts (tooltip copy). Imperative controllers remain only for PlacementController and StatusController (hint-only).
...
- **`frame-runtime.ts`** (`executeFrame()`):
  ...
  - New `isHydrating()` dep — when a Watch→Lab hydrate is in flight, the frame runtime gates **both** the snapshot reconciler AND the local physics step. Without this gate a stale worker snapshot (from the short window between `restoreState` dispatch and the first post-hydrate snapshot) would clobber the transactional hydrate's main-thread positions; without the local-step gate, the main-thread integrator would freely advance atoms that are about to be replaced wholesale.
...
WatchApp (top-level shell, useSyncExternalStore → controller snapshot)
       ...
       └── [loaded-only panels]      ← right rail (info + bonded clusters) only when loaded:
           ├── WatchTopBar           ← bottom-chrome toolbar (kind chip + short display name + link/file icon buttons)
           ├── WatchCinematicCameraToggle ← "Cinematic Camera" label + CSS toggle switch in bottom-chrome toolbar
...
- **WatchTopBar** — compact inline file-identity bar (`.watch-topbar` in `watch/css/watch.css`). Sits in the bottom-chrome toolbar strip (left side). Renders the file-kind chip, short display name (share codes formatted as grouped `XXXX-XXXX-XXXX`, timestamps as `YYYY-MM-DD HH:MM`), and two icon buttons: link (share-link inline form) and folder (local file picker).
- **WatchCinematicCameraToggle** — "Cinematic Camera" label with a CSS toggle switch (track + sliding thumb). Sits in the bottom-chrome toolbar strip (right side). Uses `aria-pressed` toggle. Props: `enabled`, `active`, `status: SnapshotCinematicCameraStatus`, `onToggle`. Status is surfaced via `data-status` attribute for CSS/test hooks.
...
User-visible flow: in Watch, the primary pill of the `WatchLabEntryControl` ("Interact From Here") opens a new Lab tab at `/lab/?from=watch&handoff=<token>`. (The caret-disclosed secondary, "Open a Fresh Lab", opens Lab without a handoff and is not part of this funnel.) Lab boots, recognizes the URL flag, skips its default C60 auto-load, consumes the handoff from `localStorage`, and runs a transactional hydrate. The hydrated scene itself is the arrival acknowledgement — there is no arrival provenance pill, banner, or any other arrival-UI surface. Hydrate success is confirmed by the rendered scene plus a `[lab.boot] watch handoff hydrated` console.info trace (extended with `velocitySource`, `unresolvedVelocityFraction`, `colorAssignmentCount`, `hasCamera`).

Canonical terminology: the Watch-side primary button is **Interact From Here** (previously called "Continue" / "Remix" / "From this frame"); the caret-disclosed secondary is **Open a Fresh Lab** (never "Open Lab" / "New Empty Lab" / "Open empty"); the localStorage-backed transport is the **handoff**; the payload inside a handoff token is the **seed** (`WatchLabSceneSeed`); per-atom velocity state is referred to as **motion state** (per-atom mass is not tracked on the wire). The popover is a **disclosure** (`aria-haspopup="true"` on the caret, `role="group"` on the popover) — NOT a menu.
...
**URL parameters:**
- `from=watch` — boot flag. Its presence (regardless of the handoff outcome) is what tells Lab to skip its default C60 auto-load. Without this gate, Lab would race the default scene load against the hydrate and the default would often win.
- `handoff=<token>` — opaque token whose value is the `localStorage` key suffix under `HANDOFF_STORAGE_PREFIX`. The payload lives in storage, not in the URL, so URLs stay short and do not end up in browser history with atom positions embedded.

Both params are scrubbed from the URL by `consumeWatchToLabHandoffFromLocation()` before it returns, regardless of outcome — a Lab reload must never re-enter the hydrate path.

**Storage schema (`src/watch-lab-handoff/watch-lab-handoff-shared.ts`):**
- Keys are prefixed with `HANDOFF_STORAGE_PREFIX` + token.
- Values are base64-encoded serialized `WatchLabSceneSeed` records (atoms + velocities + bonds + boundary + `workerConfig` + `colorAssignments` + `camera` + refined `provenance` — enough for Lab to reconstruct the frame, its orbit pose, and its authored colors without re-reading the Watch document).
- `deserializeAndValidate()` is the only reader; it returns a discriminated result (seed or structured reject reason from the `ConsumeReason` union).
- TTL is enforced lazily: on every Watch-side write, `watch-lab-handoff.ts` sweeps expired keys with the same prefix. `WatchHandoffWriteError` surfaces `kind: 'storage-unavailable' | 'quota-exceeded'`, with a quota-retry pass that runs the sweep and retries once before failing.

**Extractor boundary (Watch side):** `watch-lab-seed.ts` exposes `canBuildWatchLabSceneSeed` (predicate) and `buildWatchLabSceneSeed` (extractor). The predicate governs whether the "Interact From Here" primary is enabled; the extractor pulls the current playback frame into a seed using helpers on `watch-playback-model.ts` (`getDisplayFrameIndexAtTime`, `getTopologyFrameIdAtTime`, `canApproximateVelocityAtDisplayFrame`, `getNearestRestartFrameIdAtTime`) and reads live color assignments + live orbit camera via the optional `getColorAssignments` / `getOrbitCameraSnapshot` `BuildArgs` callbacks. The controller exposes `buildCurrentFrameLabHref()` / `openLabFromCurrentFrame()` so the React component never composes the URL or touches storage directly.

**Current-frame href cache identity (`watch-controller.ts`):** `_currentFrameHrefCache` keys on a 5-component identity tuple — `(documentKey, displayFrameKey, topologyFrameKey, restartFrameId, cameraIdentity)` — where `cameraIdentity` is a quantized `(position, target, fovDeg)` string (`POSITION_Q = 0.01 Å`, `FOV_Q = 0.5°`; `up` intentionally omitted so tiny damping drift does not churn the cache). `buildCurrentFrameLabHref` passes color + camera getters into the builder. On click, `openLabFromCurrentFrame` → `projectCurrentFrameHref` re-reads the live camera; a cache hit requires ALL 5 identity components to match. Any miss purges the stale token via `removeWatchToLabHandoff` before composing a fresh href.

**Transactional hydrate across 5 authorities, 7-phase commit (Lab side, `runtime/hydrate-from-watch-seed.ts`):**

The 5 authorities (physics, worker, renderer, scene-state, tracker+registry) are unchanged from prior revs. Rev-4 extends the commit path with two additional phases — apply camera snapshot, then apply color assignments — both covered by the same snapshot-and-rollback contract:

```
consumeWatchToLabHandoffFromLocation()    ← URL + storage read, URL scrub
        │
        ├── outcome: 'none'      → boot default scene
        ├── outcome: 'rejected'  → boot default scene + surface reason
        └── outcome: 'ready'     → enter hydrate
                    │
                    ▼
        normalize-watch-seed.ts            ← WatchLabSceneSeed → NormalizedWatchHydratePayload
                    │
                    ▼
        setHydrationActive(true)           ← opens frame-runtime lock
                    │
                    ├── snapshot 5 authorities + camera + color for rollback
                    │     1. main physics    (via cloneSceneState)
                    │     2. worker          (via existing scene-state serialization)
                    │     3. renderer        (atom set)
                    │     4. scene-state     (cloneSceneState)
                    │     5. tracker+registry (snapshot() on timeline-atom-identity + atom-metadata-registry)
                    │     + camera           (renderer.getOrbitCameraSnapshot())
                    │     + color            (appearance.snapshotAssignments())
                    │
                    ├── commit authorities using the canonical primitives
                    │     physics.clearScene + physics.appendMolecule(velocities?)
                    │     worker.clearScene  + worker.appendMolecule(velocities?) (awaited)
                    │     renderer / scene-state / tracker+registry follow
                    │     camera: applyOrbitCameraSnapshot(payload.camera)
                    │             (falls back to renderer.fitCamera() when camera===null)
                    │     color:  appearance.restoreAssignments(rebuilt assignments)
                    │             UNCONDITIONALLY on success path — empty array wipes
                    │             prior Lab state (REPLACE semantics, not additive)
                    │
                    │   any failure → restore from the snapshot tuple
                    │                  (scene-state via restoreSceneStateInPlace;
                    │                   tracker+registry via restore();
                    │                   physics/worker via the same primitives;
                    │                   camera via applyOrbitCameraSnapshot(capture.camera);
                    │                   color  via restoreAssignments(capture.colorAssignments))
                    │                  sub-failures accumulate into
                    │                  cause: { originatingCause, rollbackSubFailures }
                    │                  rather than being swallowed
                    │
                    └── setHydrationActive(false)  ← closes frame-runtime lock
```

**Camera apply contract:** `renderer.getOrbitCameraSnapshot()` reads the live OrbitControls pose (watch/js/view/watch-renderer.ts + lab/js/renderer.ts). `renderer.applyOrbitCameraSnapshot(snapshot)` on the Lab renderer cancels any in-flight camera animation, sets camera position + target + up, updates the projection matrix if `fovDeg` changed, performs a silent controls update, and recomputes focus distance. When the seed carries `camera === null` (legacy token), the hydrate falls back to `renderer.fitCamera()`.

**Color re-indexing contract:** color assignments arrive as stable-id records from Watch. The hydrate rebuilds them against the freshly-committed Lab scene by chaining Watch `atomId` → display slot (via `seed.atoms[i].id`) → Lab `atomId` (via the tracker's `assignedIds[slot]`). Atom-ids and atom-indices stay in lockstep — any assignment whose full resolution chain fails has its failing atom dropped. The rebuilt list is then installed via `appearance.restoreAssignments(...)`, which has **REPLACE semantics** (structural deep-copy; not additive): an empty array wipes any prior Lab color state.

**Worker velocity delivery (`src/types/worker-protocol.ts`, `lab/js/simulation-worker.ts`, `lab/js/worker-bridge.ts`, `lab/js/runtime/worker/worker-lifecycle.ts`):** the `appendMolecule` command gained an optional `velocities: Float64Array` (interleaved `vx,vy,vz`). The worker handler writes `engine.vel.set(velocities, atomOffset*3)` **before** `sceneVersion++` so the first post-append snapshot carries real momentum. Previously the snapshot reconciler zeroed main-thread velocities on the first `frameResult`; hydrate now delivers motion state end-to-end instead of relying on main-thread-only writes. Velocity-length mismatches warn instead of silently clipping.
...
**Worker-failure recovery carve-out:** `worker-lifecycle.restoreState` returns `Promise<{ok}>`; when it resolves `ok: false`, the worker's `onFailure` hook fires `recoverLocalPhysicsAfterWorkerFailure` in `main.ts`, which would **clobber main-thread state by reinitializing from the default scene**. During a Watch→Lab hydrate, the rollback path (above) must own authority over that same main-thread state, so `recoverLocalPhysicsAfterWorkerFailure` early-returns while `_hydrationActive` is set. The hydrate rollback is the only correct response to a failed worker mutation in this window.

**Boot ordering contract (`main.ts`):**

```
_workerInitPromise = workerLifecycle.init(…)
│
├── await _workerInitPromise                 ← must complete before hydrate consume
│
├── if (_hasPendingWatchHandoff()):
│     hydrateFromWatchSeed(outcome.seed)     ← owns authorities for the duration
│   else:
│     load default scene (_pickDefaultStructure)
│
└── fallback: after a hydrate consume returns, if physics.n === 0
    (consume was 'none' or 'rejected', or hydrate failed + rolled back to empty),
...
Lab TimelineBar (Transfer → Share tab)
    → onPublishCapsule callback (lab/js/components/timeline/timeline-transfer-dialog.tsx
                                 + lab/js/runtime/timeline/timeline-subsystem.ts)
...
User pastes any of: raw code, grouped code, /c/<code>, /watch/?c=<code>, full URL
    ...
    → WatchController.openSharedCapsule(input)  (watch/js/app/watch-controller.ts)
...
| Lab integration | `lab/js/components/timeline/TimelineBar.tsx` + `timeline-transfer-dialog.tsx`, `lab/js/runtime/timeline/timeline-subsystem.ts` (`onPublishCapsule` on `TimelineCallbacks`) | Unified "Transfer" dialog: Download / Share tabs. Renders response `warnings[]` as a subtle note. |
| Watch integration | `watch/js/app/watch-controller.ts` (`openSharedCapsule`), `watch/js/app/main.ts` | Normalize → metadata fetch → blob fetch → synthesize `File` → existing `openFile()` transactional pipeline. |
