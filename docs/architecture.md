...
│   │   │   └── history-export.ts            # V1 history file builder + capsule export: buildFullHistoryFile, buildCapsuleHistoryFile (with sparsifyInteractionTimeline), downloadHistoryFile, downloadCapsuleFile, CapsuleExportDeps; imports types from shared history module, re-exports validator
...
- **history-export.ts** — builds and downloads v1 atomdojo-history files (full + capsule). Types and validation moved to shared module (`src/history/history-file-v1.ts`); this file retains `buildFullHistoryFile()` (full envelope construction), `buildCapsuleHistoryFile()` (capsule envelope with `sparsifyInteractionTimeline` for sparse interaction data, `CapsuleExportDeps` interface), `downloadHistoryFile()` and `downloadCapsuleFile()` (programmatic anchor downloads). Re-exports shared types and `validateFullHistoryFile` for existing consumers.
