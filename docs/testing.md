...
| `turnstile-session.test.ts` | Client-side runtime under `lab/js/runtime/turnstile-session.ts`: state-machine transitions for `createTurnstileSession` (mount → solve, error-callback active/inactive paths, expired-callback active/inactive paths, load-failed via 10s watchdog), `ensureMounted` idempotence + null-detach + site-key rotation, `warm()` semantics, `disposeWidget` reuse, instance isolation, StrictMode-equivalent unmount-remount preserves token, error-retry cap of 3 then parks at `challenge-error`, retry counter resets on successful solve, pending-mount cancellation by `ensureMounted(null)`. Runs under jsdom with an injected `TurnstileApiStub` (no Cloudflare network). Frontend tsconfig. |
| `turnstile-session-dialog-integration.test.tsx` | Dialog-level integration for the controller-owned Turnstile host: structural source-scans (no `data-turnstile-host` JSX in dialog, dialog declares the empty `data-turnstile-mountpoint` slot, `QuickShareDestinationPanelProps` carries `turnstileSession` / `verificationState` / `hasToken` / `siteKey` instead of `controllerRef` / `turnstileSiteKey`); host-node identity survives a Share→Trim cascade flip; opening Quick Share injects the Cloudflare script exactly once. C1 silent-failure-fix contract: `verificationState === 'load-failed'` maps to `transfer-guest-widget-unavailable` testid + "Verification unavailable" CTA, `'challenge-error'` maps to `transfer-guest-widget-challenge-error`. Acceptance #7: TimelineBar source declares the warm-effect with deps `[turnstileSession, guestSurfaceActive, verificationState, hasGuestToken]` gated on `(ready|preparing|challenge-error)` so re-entry recovers an inactive error. `guestSurfaceActive` config gate: TimelineBar source folds `guestPublishConfig.enabled && guestPublishConfig.turnstileSiteKey` into `guestSurfaceReachable` so a signed-out user with `guestPublish.enabled=false` sees no host, mountpoint, or CTA. Frontend tsconfig (jsdom). |
...
*As of Guest Quick Share, the unit suite (Vitest — ~3299 tests across 211 files under `tests/unit/`) plus the Playwright E2E (86 specs; 9 conditional skips outside the `pages-dev` Playwright project — `poster-smoke.spec.ts` and the Guest-Quick-Share-aware `pages-dev-flows.spec.ts` among them) pass across the lab + watch + share + auth + account-erasure + handoff + capsule-preview + guest-publish surfaces. Run `npx vitest run` for the authoritative live unit total, `npm run test:e2e` for the default E2E lane, and `npm run test:e2e:pages-dev` for the pages-dev lane.*
