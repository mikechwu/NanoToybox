...
- **Local (macOS):** runs as a hard gate. `npm run test:e2e -- thumb-visual.spec.ts` compares pixels against the committed PNGs with a 2% pixel-ratio tolerance.
- **Linux / CI:** runs as a soft gate. The first run on Linux treats the missing `*-chromium-linux.png` as "baseline missing" and writes it when invoked with `--update-snapshots`. Committing the Linux baselines is an explicit choice for a future pinned CI image; without that pin, byte-identical reproducibility across runners isn't guaranteed.
