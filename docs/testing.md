...
| Lab publish/trim runtime | `capsule-publish-service.test.ts`, `trim-submit-coordinator.test.ts`, `trim-deferred-and-auth-transitions.test.ts` | Phase 1 publish/trim rebuild. `capsule-publish-service.test.ts` covers `createPreparedCapsuleService`, the two prepared executors `createPublishPreparedAccountCapsule` / `createPublishPreparedGuestCapsule`, and the extracted POST helpers `postAccountCapsuleArtifact` / `postGuestCapsuleArtifact` — with byte-identity asserts on both account and guest sides. `trim-submit-coordinator.test.ts` covers `resolveTrimSubmitTarget` resolution table plus structural assertions: account/guest executor mode-isolation; `TimelineBar` has exactly one call site per executor callback all inside the single `dispatchShareSubmit` block; coordinator does not import from `lab/js/store/app-store.ts`; surface gate `canOpenShareSurface` is mode-neutral; repo-wide negative grep that `assertPreparedCapsuleFresh` is only called from the two executor modules. `trim-deferred-and-auth-transitions.test.ts` covers Acceptance #4 deferred-measurement first-class state and Acceptance #10 sign-in mid-trim / sign-out mid-trim with `preparedArtifact` preservation. The TimelineCallbacks surface here is the renamed quintet (`onPublishFullAccountCapsule`, `onPublishFullGuestCapsule`, `onPrepareCapsuleTrim`, `onPublishPreparedAccountCapsule`, `onCancelPreparedCapsule`) plus the new `onPublishPreparedGuestCapsule(prepareId, token)`. The runtime modules under test live at `lab/js/runtime/prepared-capsule-service.ts`, `post-account-capsule.ts`, `post-guest-capsule.ts`, `publish-prepared-account-capsule.ts`, `publish-prepared-guest-capsule.ts`, `trim-submit-coordinator.ts`, and `share-auth-types.ts`. |
...
- **`postAccountCapsuleArtifact` Retry-After rendering** — numeric seconds → "try again in Ns."; HTTP-date / garbage / missing / zero / negative → generic copy; fractional seconds round up. The rendering path now flows through the extracted `postAccountCapsuleArtifact` POST helper, and the assertions live in `capsule-publish-service.test.ts` under the `postAccountCapsuleArtifact` suite.
...
Guest Quick Share added a flagged, Turnstile-gated, per-IP-quota-limited publish path that writes rows with `share_mode = 'guest'` and a 72-hour `expires_at`, swept daily by a cron. The backend surface follows the Phase 5/6/7 backend-handler conventions (hoisted `vi.fn`, SQL-dialect D1 mock, real `Request` objects, hand-built minimal context) and is typechecked under `tsconfig.functions.json`. The flag / Turnstile / quota helpers are pure modules under `src/share/` and sit on the functions tsconfig alongside the handler tests. The client-side Turnstile lifecycle (`lab/js/runtime/turnstile-session.ts`) is covered by two FRONTEND tests in the same table — `turnstile-session.test.ts` (runtime under jsdom) and `turnstile-session-dialog-integration.test.tsx` (dialog source-scan + cascade-flip behavior); each row tags its tsconfig explicitly.
...
| `turnstile-session.test.ts` | Client-side runtime under `lab/js/runtime/turnstile-session.ts`: state-machine transitions for `createTurnstileSession` (mount → solve, error-callback active/inactive paths, expired-callback active/inactive paths, load-failed via 10s `LOAD_TIMEOUT_MS` watchdog), solve watchdog (15s `EXECUTE_TIMEOUT_MS`) parks at `challenge-error` with widget reset when no Cloudflare callback settles, watchdog clears on `callback` / `error-callback` / `expired-callback` / `disposeWidget` / `resetToken`, `ensureMounted` idempotence + null-detach + site-key rotation, `warm()` semantics, `disposeWidget` reuse, instance isolation, StrictMode-equivalent unmount-remount preserves token, error-retry cap of 3 then parks at `challenge-error`, retry counter resets on successful solve and on `resetToken`, pending-mount cancellation by `ensureMounted(null)`, user-initiated recovery via `resetToken` + `warm` from `challenge-error` (no auto-rewarm on that state). Runs under jsdom with an injected `TurnstileApiStub` (no Cloudflare network). Frontend tsconfig. |
| `turnstile-session-dialog-integration.test.tsx` | Dialog-level integration for the controller-owned Turnstile host: structural source-scans (no `data-turnstile-host` JSX in dialog, dialog declares the empty `data-turnstile-mountpoint` slot, `QuickShareDestinationPanelProps` carries `turnstileSession` / `verificationState` / `hasToken` / `siteKey` instead of `controllerRef` / `turnstileSiteKey`); host-node identity survives a Share→Trim cascade flip; opening Quick Share injects the Cloudflare script exactly once. C1 silent-failure-fix contract: `verificationState === 'load-failed'` maps to `transfer-guest-widget-unavailable` testid + "Verification unavailable" CTA, `'challenge-error'` maps to `transfer-guest-widget-challenge-error` plus the user-initiated `transfer-guest-verify-retry` "Try verification again" button wired to `resetToken` + `warm`. Acceptance #7: TimelineBar source declares the warm-effect with deps `[turnstileSession, guestSurfaceActive, verificationState, hasGuestToken]` gated on `(ready|preparing)` only — `'challenge-error'` is deliberately excluded to prevent a tight retry loop with the solve watchdog. `guestSurfaceActive` config gate: TimelineBar source folds `guestPublishConfig.enabled && guestPublishConfig.turnstileSiteKey` into `guestSurfaceReachable` so a signed-out user with `guestPublish.enabled=false` sees no host, mountpoint, or CTA. Frontend tsconfig (jsdom). |
...
| `capsule-preview-perspective-bake.test.ts` | Perspective-bake invariants for Path A (publish-time pinhole): stored per-atom `rMin / rMax` equals the theoretical `K / (K+1)` at `PERSPECTIVE_K_DEFAULT`; `projectPreviewScenePerspective` and the audit-page `renderPerspectiveSketch` resolve the SAME camera on the same scene (both go through `deriveMinorAxisCamera`), verified via per-atom depth-order equivalence. Also guards `projectPreviewScene` as a function-level contract (uniform per-atom `r`) so the orthographic helper stays usable for hypothetical future structural-diagram surfaces. **The production POSTER bake is NO longer orthographic** — it switched to `projectPreviewScenePerspective` in D135 follow-up 4 (2026-04-21); poster-specific perspective invariants live in `tests/unit/capsule-preview-current-poster.test.tsx` (non-uniform stored `r` on a deep cage, rendered radii spread preserves the depth cue end-to-end). |
...
*As of Guest Quick Share, the unit suite (Vitest — ~3305 tests across 211 files under `tests/unit/`) plus the Playwright E2E (86 specs; 9 conditional skips outside the `pages-dev` Playwright project — `poster-smoke.spec.ts` and the Guest-Quick-Share-aware `pages-dev-flows.spec.ts` among them) pass across the lab + watch + share + auth + account-erasure + handoff + capsule-preview + guest-publish surfaces. Run `npx vitest run` for the authoritative live unit total, `npm run test:e2e` for the default E2E lane, and `npm run test:e2e:pages-dev` for the pages-dev lane.*
...
##### Primary-Tooltip Auto-Cue Milestone Hook

The Watch primary tooltip ("Interact From Here") fires a 5-second 1-3-1 auto-cue animation on two timeline milestones (50% and 100%) at most once per file each. The semantics — once-per-file (resets on `fileIdentity` change), arm-then-fire (milestone must be observed with `currentTimePs < threshold` before it can fire; deep-links that resume past a threshold do NOT cue), and paused-seek coalescing (a 10%→95% scrub while paused fires the end cue only) — all live in `watch/js/hooks/use-timeline-milestone-tokens.ts`. The token-driven DOM state on the consuming component (`primaryAutoCueToken` → `data-auto-cue` on the tooltip) is exercised by the **auto-cue DOM state** describe block at the end of `tests/unit/watch-lab-entry-control.test.tsx`, which pairs `vi.useFakeTimers()` with a controlled `rerender(...)` to bump the token without real time, then advances timers via `vi.advanceTimersByTime(durationMs + slack)` to prove the clear path. Two recipes worth reusing for similar timer-driven token hooks:

- **Pair `vi.useFakeTimers()` with `rerender(...)`** to drive a token-bump effect deterministically — a controlled rerender models the production token-change path while the fake-timer clock proves the clear-after-duration timeout fires once and only once.
- **Prefer `screen.getByRole('tooltip')` over class-string selectors** when the production JSX already sets `role="tooltip"` (e.g., `WatchLabEntryControl`'s primary tooltip). The role accessor proves both presence AND the role attribute in a single query, and is resilient to class renames.

End-to-end visibility under the touch-interaction predicate (`isTouchInteraction()` parity) is exercised at the integration level by the mobile auto-cue describe at the end of `tests/e2e/watch-lab-entry.spec.ts` — see the "Touch-context emulation" and "Arm-then-fire React effects" entries under [E2E Conventions Reinforced by This Spec](#e2e-conventions-reinforced-by-this-spec) above. The shared lower-level primitive `src/ui/use-timed-cue.ts` (`useTimedCue({ triggerToken, durationMs })`) remains a candidate for direct unit coverage.
...
- **Touch-context emulation via `test.use`** — for specs that depend on the touch-interaction predicate (`isTouchInteraction()` parity, mobile-only visibility branches), apply the touch context to every case in a describe block via `test.use({ hasTouch: true, isMobile: true, viewport: { width: 375, height: 812 } })`. Precedent: the mobile auto-cue describe at the bottom of `tests/e2e/watch-lab-entry.spec.ts:655` (Watch → Lab primary hint), and the older single-context precedent at `tests/e2e/timeline-layout.spec.ts:154`. To guard against a future browser-emulation regression silently exercising the desktop branch, tests whose visibility depends on the predicate should pin both halves at the top of the body — `matchMedia('(pointer: coarse)').matches` AND `!matchMedia('(hover: hover)').matches` — so an emulation-context drop fails loudly instead of passing for the wrong reason.
- **Arm-then-fire React effects via `_watchScrub`** — `useTimelineMilestoneTokens` at `watch/js/hooks/use-timeline-milestone-tokens.ts` is arm-then-fire: the effect must observe a snapshot with `currentTimePs < midPs` before it can fire the token bump. When driving such effects from Playwright via a state hook like `_watchScrub`, yield exactly one `requestAnimationFrame` between the arm call (a value below the threshold) and the fire call (a value above it) so React commits the arming snapshot — React 18 schedules effects via microtasks that run before rAF callbacks, so a single rAF is sufficient. Add a 1-ps (or equivalent) slack on the fire value so future float-precision drift in the player cannot silently skip the crossing. Precedent: the `scrubWatch` helper in the same mobile auto-cue describe at `tests/e2e/watch-lab-entry.spec.ts:661`.
