...
- Safety guards: per-atom velocity hard cap (`vHardMax`), per-atom thresholded smooth saturation for internal forces (Tersoff+wall), smooth saturation for interaction forces (drag/translate/rotate) at the spring level, and a transitional elevated KE cap (Phase 3/4, will be removed after saturation validation)
