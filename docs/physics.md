...
- **Cell-list spatial acceleration** — `buildNeighborList()` and `updateBondList()` use linked-list cell grids instead of O(N²) all-pairs scans. Neighbor grid uses 2.60 Å cells; bond grid uses 1.8 Å cells. Shared `_buildCellGrid()` helper. Validated via `page/bench/bench-celllist.html`. Sparse-domain optimization: `_buildCellGrid()` tracks occupied cells and clears only those on the next rebuild (O(N) instead of O(nCells)), preventing cubic slowdown when atoms spread far apart. Validated via `page/bench/bench-celllist.html` (equivalence) and `page/bench/bench-spread.html` (span-independence).
