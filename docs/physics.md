...
- **Topology builders** — `src/topology/build-bond-topology.ts` provides three entry points: `buildBondTopologyFromAtoms` (loader path, also used by the capsule-preview publish path), `buildBondTopologyFromPositions` (Watch reconstruction), and `buildBondTopologyAccelerated` (physics hot path with output-buffer reuse and preallocated workspace).
- **Preview + physics consolidation.** The capsule-preview publish path (`src/share/publish-core.ts`) now derives its 3D bond list with the same builder used by the physics hot path: `buildBondTopologyFromAtoms` + a `BondRuleSet` from `createBondRules`. The previous preview-local `deriveBondPairs` helper has been replaced. When the capsule file carries a `bondPolicy`, the preview honors its `cutoff` / `minDist`; otherwise it falls back to carbon defaults (cutoff 1.85 Å, minDist 0.5 Å — the preview fallback is intentionally slightly looser than `BOND_DEFAULTS.cutoff = 1.8` Å to be visually tolerant of off-equilibrium capsule frames).
...
**Internal force saturation (`saturateInternalForces`):** After computing Tersoff and wall forces, per-atom thresholded smooth saturation bounds pathological force spikes. Forces below `F_REPULSION_START` (`CONFIG.physics.fRepulsionStart`, default 40 eV/Å) pass through unchanged. Above that threshold, forces are smoothly compressed toward `F_MAX_INTERNAL` (`CONFIG.physics.fMaxInternal`, default 80 eV/Å) using the formula `f_eff = start + excess / (1 + excess / headroom)`. This is per-atom (not global scaling), runs inside `computeForces()` after Tersoff+wall but before interaction forces, and protects against overlap/placement force spikes without affecting normal Tersoff dynamics.
...
`lab/js/simulation-worker.ts` runs `PhysicsEngine` on a dedicated Web Worker thread. The main thread communicates via `lab/js/worker-bridge.ts` using a typed `WorkerCommand` / `WorkerEvent` message protocol (`src/types/worker-protocol.ts`). Worker lifecycle (creation, init, stall detection, teardown) is managed by `lab/js/runtime/worker/worker-lifecycle.ts`. Worker snapshot reconciliation (position sync, atom-remap, bond refresh) is owned by `lab/js/runtime/worker/snapshot-reconciler.ts`. If the worker fails to initialize or stalls (5s warning sets stalled flag, 15s fatal triggers sync fallback), the engine falls back to synchronous `PhysicsEngine` on the main thread. Both paths use identical `physics.ts` code.
