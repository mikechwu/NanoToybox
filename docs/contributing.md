...
- Watch Round 6 — Interpolation runtime: shared physical constants (`src/history/units.ts`), interpolation runtime with extension-oriented strategy registry (`watch/js/playback/watch-trajectory-interpolation.ts`), three built-in strategies (Linear stable, Hermite + Catmull-Rom experimental), per-bracket capability layer (`InterpolationCapability` from importer), cursor-cache fast path for sequential playback, unified pipeline rule (`applyReviewFrameAtTime()` is sole caller of `interpolation.resolve()` + `renderer.updateReviewFrame()`), interpolation mode types in settings (`WatchInterpolationMode`, `PRODUCT_INTERPOLATION_MODE_IDS`), import diagnostics (`ImportDiagnostic`), CSS tokens scoped to `.watch-workspace`
...
- Playback Capsule (Phases 1-4) — capsule file format and shared types in `src/history/history-file-v1.ts`, capsule importer (`watch/js/document/capsule-history-import.ts` replacing `reduced-history-import.ts`), appearance export/import, sparse interaction data contract, Lab capsule export builder with capsule/full export kinds (replay removed from export UI), stable-ID appearance model (Lab renderer and export use stable `atomId`s), golden parity validation
...
- **Timeline hint copy lives in `timeline-hints.ts`.** All tooltip text for timeline controls (record, review, restart, clear) is defined as constants in `lab/js/components/timeline/timeline-hints.ts`. Do not scatter timeline hint copy inline in components -- import from `TIMELINE_HINTS`.
...
The Watch→Lab hydrate module (`lab/js/runtime/handoff/hydrate-from-watch-seed.ts`) is the exemplar. Any code that mutates scene state across both main-thread physics and the worker — and must be atomic from the user's perspective — follows this shape:
...
**Watch seed builder** (`watch/js/handoff/watch-lab-seed.ts`): `BuildArgs` accepts optional `getColorAssignments` and `getOrbitCameraSnapshot` closures. Per-atom velocity source tracking collapses to a single tag matching `WatchLabVelocitySource`. `unresolvedVelocityFraction` is computed pre-null-promotion. Unknown `atomId`s in color assignments are dropped at build time with a `console.warn`.

**Watch controller cache identity** (`watch/js/app/watch-controller.ts`): `_currentFrameHrefCache` uses a 5-tuple identity that includes a `cameraIdentity` component — a quantized `(position, target, fovDeg)` string with `POSITION_Q = 0.01`, `FOV_Q = 0.5`. `buildCurrentFrameLabHref` passes both color and camera getters into the seed builder. The entry-control handlers (primary "Interact From Here" and secondary "Open a Fresh Lab") funnel through `projectCurrentFrameHref`, which re-reads the live camera on click; a cache hit requires **all 5** identity components to match, and a miss purges the stale token via `removeWatchToLabHandoff`.

**Lab hydrate transaction** (`lab/js/runtime/handoff/hydrate-from-watch-seed.ts`): follows the Transactional Scene Mutation Pattern above. Color re-indexing maps Watch `atomId` → display slot (via `seed.atoms[i].id`) → Lab `atomId` (via tracker `assignedIds[slot]`); atoms whose full resolution chain fails are dropped. `[lab.boot] watch handoff hydrated` extends with `velocitySource`, `unresolvedVelocityFraction`, `colorAssignmentCount`, and `hasCamera`.

**Worker velocity delivery** (`src/types/worker-protocol.ts`, `lab/js/simulation-worker.ts`, `lab/js/worker-bridge.ts`, `lab/js/runtime/worker/worker-lifecycle.ts`): the `appendMolecule` command carries an optional `velocities: Float64Array`. The worker writes `engine.vel.set(velocities, atomOffset*3)` **before** `sceneVersion++` so the first post-append snapshot reflects real motion state — the snapshot reconciler previously zeroed main-thread velocities on the first frameResult. Velocity-length mismatches warn rather than silently clip.

**Bonded-group appearance runtime** (`lab/js/runtime/bonded-groups/bonded-group-appearance-runtime.ts`): exposes `snapshotAssignments()` and `restoreAssignments(prev)` for transactional capture/restore. Both use structural deep-copy and REPLACE semantics (not additive) — restoring an empty snapshot wipes prior state, which is what the hydrate success path relies on.

**E2E observability hooks:**
- `_getWatchCameraSnapshot` (`watch/js/app/main.ts`, gated on `?e2e=1`) — returns the live Watch orbit pose.
...
The Lab timeline-transfer button label and tooltip are both **"Share & Download"** — do not reintroduce "Transfer history", "Export", or standalone "Share". `TRANSFER_HINT_COPY` is exported from `lab/js/components/timeline/timeline-transfer-dialog.tsx` so tests can pin the copy.
...
The placement solver (`lab/js/runtime/placement/placement-solver.ts`) uses a 3-surface architecture that must be kept in sync. The module header documents this, but the key constraint is:
...
The placement camera framing system (`lab/js/runtime/placement/placement-camera-framing.ts`) is a pure solver with no THREE/renderer/store imports. When changing placement framing behavior:
...
The interpolation runtime (`watch/js/playback/watch-trajectory-interpolation.ts`) is designed for extension. To add a new method:
...
3. **For product-visible methods:** add the new ID to `PRODUCT_INTERPOLATION_MODE_IDS` in `watch/js/settings/watch-settings.ts` and use `availability: 'product'` in the metadata. The settings picker reads this array.
...
E2E camera-snapshot hooks: `_getWatchCameraSnapshot` is gated on `?e2e=1` (`watch/js/app/main.ts`); `_getLabCameraSnapshot` is **unconditional** (`lab/js/main.ts`) because the Lab tab opened from Watch via the handoff (whether "Interact From Here" or "Open a Fresh Lab") carries no `?e2e` param, and the camera-continuity spec in `tests/e2e/watch-lab-entry.spec.ts` needs to read it on arrival. The E2E spec accesses the secondary action by first clicking `.watch-lab-entry__caret` to open the disclosure popover.
...
| Timeline components & hints | `lab/js/components/timeline/TimelineBar.tsx`, `lab/js/components/timeline/timeline-hints.ts`, `lab/js/components/timeline/timeline-mode-switch.tsx`, `lab/js/components/timeline/timeline-clear-dialog.tsx`, `lab/js/components/timeline/timeline-format.ts` |
| Web Worker / bridge | `lab/js/simulation-worker.ts`, `lab/js/worker-bridge.ts`, `src/types/worker-protocol.ts` |
| Runtime modules (scene, worker, input) | `lab/js/runtime/scene-runtime.ts`, `lab/js/runtime/worker/worker-lifecycle.ts`, `lab/js/runtime/worker/snapshot-reconciler.ts`, `lab/js/runtime/interaction/input-bindings.ts`, `lab/js/runtime/interaction/interaction-dispatch.ts` |
| Overlay layout & open/close policy | `lab/js/runtime/overlay/overlay-layout.ts`, `lab/js/runtime/overlay/overlay-runtime.ts` |
| Focus resolution & onboarding | `lab/js/runtime/camera/focus-runtime.ts`, `lab/js/runtime/onboarding.ts`, `lab/js/components/OnboardingOverlay.tsx` |
...
| Bonded clusters (panel + highlight + color) | `lab/js/runtime/bonded-groups/bonded-group-runtime.ts`, `lab/js/runtime/bonded-groups/bonded-group-highlight-runtime.ts`, `lab/js/runtime/bonded-groups/bonded-group-appearance-runtime.ts`, `lab/js/runtime/bonded-groups/bonded-group-coordinator.ts`, `lab/js/components/BondedGroupsPanel.tsx`, `lab/js/runtime/interaction/interaction-highlight-runtime.ts` |
...
| Watch controller & domains | `watch/js/app/watch-controller.ts`, `watch/js/playback/watch-playback-model.ts`, `watch/js/analysis/watch-bonded-groups.ts`, `watch/js/view/watch-view-service.ts`, `watch/js/analysis/watch-bonded-group-appearance.ts`, `watch/js/view/watch-camera-input.ts`, `watch/js/view/watch-overlay-layout.ts`, `watch/js/settings/watch-settings.ts`, `watch/js/playback/watch-trajectory-interpolation.ts` |
| Watch interpolation | `watch/js/playback/watch-trajectory-interpolation.ts` (strategy registry + built-in strategies), `watch/js/settings/watch-settings.ts` (`WatchInterpolationMode`, `PRODUCT_INTERPOLATION_MODE_IDS`), `watch/js/document/full-history-import.ts` (`InterpolationCapability`, `ImportDiagnostic`), `src/history/units.ts` (physical constants) |
...
| Scene / placement | `lab/js/scene.ts`, `lab/js/placement.ts`, `lab/js/runtime/placement/placement-solver.ts`, `tests/unit/placement-solver.test.ts` |
