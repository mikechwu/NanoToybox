...
3. **Independent version constants.** `TEMPLATE_VERSION` in `src/share/capsule-preview.ts` controls the dynamic-poster cache key (bumped 1→2 at V2 launch; bumped 2→3 on 2026-04-21 when the poster renderer retargeted from `scene.thumb` back to `scene.atoms`/`scene.bonds`). `CURRENT_THUMB_REV` in `src/share/capsule-preview-scene-store.ts` is a SEPARATE constant versioned independently so thumb-algorithm changes (sampler, margin math, visibility filter) do not force a poster cache invalidation and vice versa. V2 launched at `CURRENT_THUMB_REV = 2`; the live value tracks subsequent D138 follow-ups and the 2026-04-21 D135 follow-up (currently `15` — see D135 follow-up 3 for the reverted `16 → 15` bump that piggy-backed a stale-rebake onto the thumb constant).
4. **Content-bound dynamic-poster ETag.** The ETag rebinds to the canonical scene hash: `"v${TEMPLATE_VERSION}-<FNV1a32 of [TEMPLATE_VERSION, scene.hash, sanitizedTitle, shareCode]>"` (`functions/api/capsules/[code]/preview/poster.ts::dynamicPosterETag`). Currently emits `"v4-<8hex>"`. Any observable input to the render changes the ETag; no observable input change leaves a stale cache entry valid.
...
8. **Account-page lazy rebake (2026-04-21 follow-up).** The account-list endpoint's lazy heal (originally scoped to bondless rows only) now also rebakes rows that are missing (`preview_scene_v1 IS NULL`), malformed (parse-failed), or stale-rev (`thumb.rev < CURRENT_THUMB_REV`). Start cap per request is `HEAL_START_CAP_FIRST_PAGE = 8` on page 1 and `HEAL_START_CAP_CURSORED = 5` on paged requests. **Claim attempts** are separately bounded at `HEAL_CLAIM_ATTEMPT_CAP_FIRST_PAGE = 16` / `HEAL_CLAIM_ATTEMPT_CAP_CURSORED = 10` (2× the start caps) so nomination cost stays predictable even when the whole page is already lease-held by another tab. A new D1 column `preview_rebake_claimed_at` (migration `0010_capsule_share_preview_rebake_claim.sql`) provides cross-request/cross-tab dedup via a 90 s TTL lease, atomically claimed with `UPDATE … WHERE id = ? AND (claim IS NULL OR claim < ?)` and accepted only when `changes === 1`. The claim path is **fail-open**: a UPDATE that throws emits `[account.capsules] heal-claim-failed share=… error=…`, the offending row is skipped, and the capsule-list response still returns 200 with a (possibly partial) `previewPending`. `HealResult` gains a `persisted: boolean` field that reflects real D1 mutation (`meta.changes >= 1`), so batch metrics distinguish rebaked-in-memory from committed-to-D1 even when a concurrent delete race leaves the UPDATE matching zero rows. Client auto-refresh (8 s post-response timer + `visibilitychange` after ≥ 30 s hidden) is **first-page-only**: once the user has invoked "Load more" in the current session, auto-refresh is suppressed until the next `reload()` so a timer-driven page-1 refetch cannot collapse paginated state. `onLoadMore()` sets the suppression latch AND aborts any in-flight page-1 refresh BEFORE awaiting the cursor fetch — otherwise a timer or visibility refresh could start a new page-1 fetch during the pagination window and silently collapse the paginated list. On pagination failure the latch is rolled back so convergence resumes in the page-1 regime. `reload()` is sequence-guarded (`reloadSeqRef`) so a slow prior load cannot overwrite a newer transition. **Non-goal:** stored R2 poster blobs (`preview_poster_key`) remain immutable-served and are NOT invalidated by this path; bulk repair remains the admin backfill endpoint's job.

9. **Poster-scene rev contract + share-page self-heal (2026-04-21 follow-up 3).** A live-URL audit of `/api/capsules/:code/preview/poster?v=t2` surfaced two intertwined bugs on the OG poster:

   a. **Source drift.** `CurrentPosterSceneSvg` had been consuming `preview_scene_v1.thumb` (the perspective thumb bake) as its primary geometry source while the publish-time contract in `publish-core.ts` still documented the poster scene as orthographic and the thumb as perspective-only. The thumb bake's publish-time visibility filter (calibrated for a 100×100 thumb viewBox) dropped bonds that landed under ~3 units at that scale — and those dropped bonds are absent from `thumb.bonds` entirely. Blown up to 600×600 the effect is a poster missing wiring the poster scene always had.

   b. **Anisotropic normalization.** The poster scene itself was projected at `targetWidth: 600, targetHeight: 500` (non-square), then `buildPreviewSceneV1` normalized via `x/600, y/500` — a non-uniform division that squashed x by 1.2× vs. y. Any renderer treating stored coords as isotropic rendered every structure 1.2× taller than wide. The thumb bake (separate code path) was already square (500 × 500) and correct; the bug hid under the thumb source-drift above and surfaced once the poster renderer was retargeted to `scene.atoms`.

   **Decisions:**

   - Renderer now reads `scene.atoms` / `scene.bonds` as primary; falls back to `scene.thumb` ONLY when the poster scene carries no bonds (legacy pre-D138 bondless rows). Source split matches the publish-core contract again.
   - Poster projection target becomes SQUARE (`600 × 600`) to match the thumb's `500 × 500` fix and give `buildPreviewSceneV1` isotropic bounds.
   - `SCENE_ATOM_CAP` lifted `32 → 5000`, `SCENE_BOND_CAP` lifted `64 → 10000` — the orthographic poster scene now operates at the same effective-uncapped fidelity as the thumb bake, so dense multi-component capsules (100s of atoms across multiple molecules) render every bond.
   - **New `CURRENT_SCENE_REV = 1` and `PreviewSceneV1.rev` field** — poster-scene invalidation is no longer proxied through `CURRENT_THUMB_REV`. The two constants are now strictly scoped: thumb-algorithm changes → `CURRENT_THUMB_REV`; poster-scene-bake changes → `CURRENT_SCENE_REV`. Missing `rev` parses as rev 0 (always stale) so legacy rows drain naturally. A prior iteration's shortcut of bumping `CURRENT_THUMB_REV = 15 → 16` to piggy-back the stale-rebake was reverted — it forced thumb rebakes whose output was byte-identical, which wasted backfill cost and muddied the rev-history audit trail.
   - **Poster route (`functions/api/capsules/[code]/preview/poster.ts`) now self-heals stale scenes synchronously**, parallel to the existing `sceneMissing` and `sceneBondless` heal paths. Classification: `scene.rev < CURRENT_SCENE_REV` (or missing `rev`) triggers `rebakeSceneFromR2(..., {overwrite: true})` before render. Logs `scene-stale-rebaked` on success, `scene-stale-heal-failed` on failure with the stale scene served (not terminal fallback) — public share posters self-heal on first hit without requiring an account-page visit or admin backfill. This was the actual production gap: without it, only owner-visited rows converged.
   - `TEMPLATE_VERSION` bumps `2 → 3` to invalidate every cached social unfurl.

   **Account-endpoint stale-rev classifier** (bullet 8 above) now reads both `scene.rev < CURRENT_SCENE_REV` OR `scene.thumb.rev < CURRENT_THUMB_REV`; either condition triggers the same rebake (which refreshes both surfaces atomically since they share one D1 UPDATE).

   **Regression coverage:** `tests/unit/capsule-preview-current-poster.test.tsx` locks the rendered aspect ratio + source-split contract on the shipped `CurrentPosterSceneSvg` component; `tests/unit/poster-endpoint.test.ts` adds stale-rev rebake-path cases (scene without `rev`, scene at obsolete `rev`, fresh-rev no-op, heal-failure fallthrough to served-stale). `capsule-preview-poster-figure.test.ts` continues to cover the audit-workbench sketch path independently.

10. **Poster scene becomes perspective (2026-04-21 follow-up 4).** A post-mortem of follow-up 3 flagged that swapping the poster back to `scene.atoms`/`scene.bonds` landed on the orthographic bake while `CurrentPosterSceneSvg`'s renderer was still perspective-aware (`perspectiveMultiplier(a.r, rMedian)` at line 243). Under an orthographic bake every atom has identical stored `r`, so the multiplier collapsed to ≈ 1 and the depth-cue code became a no-op. The product intent is consistent perspective across BOTH surfaces (profile thumb + OG poster), not "structural diagram" for one and "3D figure" for the other.

   **Decision:** poster scene bake switches from orthographic (`projectPreviewScene`) to pinhole perspective (`projectPreviewScenePerspective`, same `PERSPECTIVE_K_DEFAULT = 3.17` as the thumb). Square 600×600 target preserved (aspect invariant from follow-up 3 stays intact). `CURRENT_SCENE_REV` advances `1 → 2`. `TEMPLATE_VERSION` advances `3 → 4` to invalidate every cached social unfurl. The `perspectiveMultiplier` renderer code stops being dead.

   **Alternatives considered:** (a) Introduce a separate `posterPerspective` field so the orthographic scene stays as a structural-diagram variant. Rejected — no consumer wanted the orthographic variant, so keeping it would be dead schema + dead storage cost for every row. (b) Strip `perspectiveMultiplier` from the renderer to match the orthographic source. Rejected — that would codify the product inconsistency (thumb has depth cue, poster does not) as a contract, exactly the state the user called out as wrong.

   **Regression coverage:** new `tests/unit/capsule-preview-current-poster.test.tsx` cases assert (a) stored per-atom `r` values are non-uniform on a deep 3D fixture (`maxR/minR > 1.10` on a C60 cage) and (b) rendered circle radii preserve that spread, so a future regression that either reverts the bake to orthographic or strips the renderer multiplier trips one of the two invariants. `tests/unit/capsule-preview-perspective-bake.test.ts` was reframed from "orthographic poster path remains uniform-radius" (a product contract) to "`projectPreviewScene` (the function) emits uniform-radius atoms" (a function-level property) so the helper stays usable for hypothetical future structural-diagram surfaces without lying about which surface consumes it today.
...
**NOT in this ADR:** any one-shot operational cleanup (e.g. invalidating surviving V1 stored posters on the V1→V2 cutover). That category is rollout operations, not steady-state design — no such script was needed at launch because `TEMPLATE_VERSION` cache-busts the dynamic path (bumped to 3 on 2026-04-21 for the poster source swap) and stored-V1 posters age out naturally via `CURRENT_THUMB_REV`-gated rebakes. Runbooks for future pipeline bumps live in `docs/operations.md`.

**Related ADRs:** D130 (V1 OG render stack — still in force; V2 uses the same Vercel OG plugin / bundled-font pipeline, only the scene source changed). D132 (`posterUrl` semantics — unchanged; "endpoint exists" still applies, and V2 narrows the "dynamic vs. stored" discriminator because every accessible capsule now has a pre-baked scene). D133 (two-axis `?v=` cache key — `?v=t4` now reflects `TEMPLATE_VERSION = 4`; the scheme is unchanged). D134 (`CAPSULE_PREVIEW_DYNAMIC_FALLBACK` allowlist flag — still the rollback lever for the dynamic poster path; V2 did not retire it).

**Evidence:** `src/share/capsule-preview.ts` (`TEMPLATE_VERSION = 4`, history documents the 2→3→4 progression through the 2026-04-21 follow-ups); `src/share/capsule-preview-scene-store.ts` (`CURRENT_THUMB_REV` — currently `15`, scoped strictly to thumb-payload shape; `CURRENT_SCENE_REV` — currently `2`, scoped strictly to poster-scene bake shape; `SCENE_ATOM_CAP = 5000`, `SCENE_BOND_CAP = 10000`; `PreviewSceneV1.rev`; `PreviewStoredThumbV1`; `rev >= CURRENT_SCENE_REV` / `rev >= CURRENT_THUMB_REV` fast-path gates); `src/share/capsule-preview-current-poster.tsx` (poster renderer consumes `scene.atoms`/`scene.bonds` under the perspective bake — `perspectiveMultiplier(a.r, rMedian)` produces the depth cue); `src/share/publish-core.ts` (poster path calls `projectPreviewScenePerspective` at 600×600 since follow-up 4); `migrations/0009_capsule_preview_scene_v1.sql`; `functions/api/capsules/[code]/preview/poster.ts::dynamicPosterETag` (`"v${TEMPLATE_VERSION}-${fnv1a32Hex(...)}"`).
...
**Scope caveat (important):** "bonded cluster" here means a **connected component of the preview proximity graph** (derived from `bondPolicy.cutoff` + `bondPolicy.minDist`), NOT authoritative molecular connectivity. The capsule file format does not carry dense-frame topology, and the publish-time bond builder (`buildBondTopologyFromAtoms` with a single-cutoff `createBondRules({ cutoff, minDist })` — replaced the prior `deriveBondPairs` call during the D138 follow-up work; byte-identical output for carbon-only scenes) produces a pure distance-cutoff graph. Close-approach frames (mid-flight collisions, solute-solvent encounters) can fuse two unbonded molecules into one cluster, and the dominance guard cannot unfuse them. This is a deliberate tradeoff; a chemistry-aware upgrade is out of scope — see the `makeCloseApproachCapsule` regression fixture which locks in the observed fuse/separate behavior at two different cutoffs.
...
**Decision:** The Turnstile widget mounts with `appearance: 'interaction-only'` and its container renders at zero footprint by default. No "Verifying you're human…" placeholder is drawn ahead of Cloudflare's own decision. If Cloudflare elects to present a visible challenge, the container is expanded at that moment. The widget is also rendered with `execution: 'execute'` so mount is cheap (no solve at render time); a deliberate `warm()` call from the host triggers the actual solve — see `lab/js/runtime/turnstile-session.ts` (`createTurnstileSession`).

**Rationale:** In `interaction-only` mode Turnstile is invisible for legitimate traffic and only renders a visible challenge for requests Cloudflare scores as suspicious. An earlier iteration reserved a permanent visible slot with a placeholder string; users with clean traffic never saw the placeholder resolve (because no challenge was ever raised) and interpreted the frozen "Verifying…" label as a broken CTA. Zero-footprint by default inverts the signal — a visible challenge now means Cloudflare actually wants one — and the CTA itself stays responsive for the happy path. Pairing `interaction-only` with `execution: 'execute'` keeps the mount free of any solve-side cost, so reparenting the same widget across guest surfaces (D154) does not re-spend a challenge.
...
**Decision:** If the Turnstile script fails to load (ad blocker, restrictive proxy, offline, CSP mismatch), the Quick Share CTA label switches to "Verification unavailable" (testid `transfer-guest-widget-unavailable`) and a message points the user to the OAuth sign-in path as an alternative. The button is disabled until the widget either loads or the user navigates to the account path. The 10-second load watchdog that drives the `'load-failed'` state is owned by `lab/js/runtime/turnstile-session.ts` (`LOAD_TIMEOUT_MS`) and surfaced to the UI via `verificationState`.
...
**Related ADRs:** D143 (interaction-only mode); D154 (session-scoped runtime that owns the watchdog and the `verificationState` machine).
...
## D153: Three-Layer Mode-Neutral Publish Architecture for Capsule Trim (Phase 1 Rebuild)

**Status:** Accepted.

**Decision:** Capsule trim publish is rebuilt into three strictly separated layers with mode-neutral preparation:

1. **Selection / UI layer.** `lab/js/components/timeline/TimelineBar.tsx` owns trim entry kind (`manual` vs `oversize`), measurement policy (`deferred` vs `required`), selection state, and a single `dispatchShareSubmit` helper. It does NOT own fetch, auth, Turnstile, prepared-cache policy, or submit-target pinning. The four executor callbacks (`onPublishFullAccountCapsule`, `onPublishFullGuestCapsule`, `onPublishPreparedAccountCapsule`, `onPublishPreparedGuestCapsule`) each appear exactly once, all inside the dispatcher's switch — pinned by `tests/unit/trim-submit-coordinator.test.ts`.
2. **Preparation layer.** `lab/js/runtime/prepared-capsule-service.ts` is mode-neutral and the sole owner of `assertPreparedCapsuleFresh(prepareId)`. It exposes `prepareCapsulePublish`, `cancelPreparedPublish`, `assertPreparedCapsuleFresh`, and `getPreparedCapsuleArtifact`, and does NOT POST. Prepared bytes are target-neutral: the same `prepareId` can flow to either the account or guest executor.
3. **Publish layer.** Split into per-mode POST helpers (`post-account-capsule.ts`, extracted from the old `publish-capsule-artifacts.ts`, and `post-guest-capsule.ts`, renamed from `publish-guest-artifact.ts`) that solely own mode-specific 4xx/5xx error mapping; per-mode prepared executors (`publish-prepared-account-capsule.ts`, `publish-prepared-guest-capsule.ts`) that each call `assertPreparedCapsuleFresh` then the matching POST helper; and a pure submit coordinator `trim-submit-coordinator.ts` whose `resolveTrimSubmitTarget(action, authStatus, publicConfig, turnstileToken)` returns `{ kind: 'ok', target } | { kind: 'unavailable', reason }` with reasons `auth-required | unverified | guest-disabled | config-missing | verification-required`. The coordinator is the sole seam that decides the `TrimSubmitTarget` from action + auth + config + token, and is invoked exactly once per submit click.

`AuthStatus` and `PublicConfig` types live in `lab/js/runtime/share-auth-types.ts` (dependency-free) so the coordinator does not import from the Zustand store layer; `lab/js/store/app-store.ts` re-exports for back-compat. `TimelineCallbacks` is renamed to make the architecture explicit (`onPublishCapsule` → `onPublishFullAccountCapsule`; `onConfirmGuestShare` → `onPublishFullGuestCapsule`; `onPrepareCapsulePublish` → `onPrepareCapsuleTrim`; `onPublishPreparedCapsule` → `onPublishPreparedAccountCapsule`; new `onPublishPreparedGuestCapsule(prepareId, token)`; `onCancelPreparedPublish` → `onCancelPreparedCapsule`). The share surface entry gate is `canOpenShareSurface = hasRange && (onPublishFullAccountCapsule || onPublishFullGuestCapsule)`, so a future configuration that wires only guest still lights up the surface.

**Rationale:** Phase 0 bugs were drift bugs — submit-target inference inlined inside each handler diverged across surfaces. The fix is structural: target resolution moves to a single pure function with a typed unavailable-reason ADT, snapshot recheck moves to a single owner called once before any network I/O, and POST/error-mapping is partitioned per mode so neither executor can leak the other's error vocabulary. Mode-neutral preparation is the load-bearing detail: prepared bytes have to be target-neutral so the user can change Share action between prepare and submit (e.g., sign in mid-flow) without re-preparing. The coordinator also subsumes guest preflight — the previous "if guest and no token then early-return" branch in `TimelineBar` becomes a `verification-required` reason returned from `resolveTrimSubmitTarget`, removing the last surface-side branch on auth/config.

**Alternatives rejected:**

- A single `publishPreparedCapsule(prepareId, mode, token?)` callback overloaded on mode + nullable token (drifts mode semantics into one signature; reintroduces the guest-vs-account branch that the rebuild deletes).
- Inlining target inference inside each handler (the architecture this rebuild fixes — drift across surfaces is exactly what created Phase 0's bugs).
- Sharing executor scaffolding across modes (would force one executor module to import the other's error types and break per-mode error-mapping isolation).

**Evidence:** `lab/js/runtime/prepared-capsule-service.ts`, `lab/js/runtime/post-account-capsule.ts`, `lab/js/runtime/post-guest-capsule.ts`, `lab/js/runtime/publish-prepared-account-capsule.ts`, `lab/js/runtime/publish-prepared-guest-capsule.ts`, `lab/js/runtime/trim-submit-coordinator.ts`, `lab/js/runtime/share-auth-types.ts`; `lab/js/store/app-store.ts` (`TimelineCallbacks`); `lab/js/components/timeline/TimelineBar.tsx` (`dispatchShareSubmit`, `surfaceUnavailable`, `canOpenShareSurface`); tests `tests/unit/capsule-publish-service.test.ts`, `tests/unit/trim-submit-coordinator.test.ts`, `tests/unit/trim-deferred-and-auth-transitions.test.ts`.

**Related ADRs:** D88 (validate-then-write publish model — the prepared-capsule-service preserves byte-identity end-to-end); D139 (separate guest endpoint — Phase 1 makes the lab-side path mode-neutral so prepared bytes can flow to either endpoint).

## D154: Session-Scoped Turnstile Runtime Owned by TimelineBar, Not the Destination Panel

**Status:** Accepted, 2026-04-26.

**Context:** The original guest Quick Share lifecycle (script load, widget render, token, expiry refresh, error handling) lived inside `QuickShareDestinationPanel`. Because the panel unmounts and remounts on cascade flips between Share and Trim guest surfaces inside one dialog session, every flip tore down the widget, dropped the live token, and forced a fresh solve — even though the user had already verified once in the same session. React-owned mount also meant the host element re-rendered on parent state changes, and the widget's imperative DOM occasionally fought the reconciler. A `GuestTurnstileController` interface plus a `guestTurnstileControllerRef` had grown around the panel to paper over this, but the lifecycle ownership was still surface-local.

**Decision:** Client-side Turnstile lifecycle moves out of `QuickShareDestinationPanel` into a session-scoped runtime: `lab/js/runtime/turnstile-session.ts` exposes `createTurnstileSession({ onStateChange, onTokenChange }, api?)` returning a `TurnstileSessionController` with closure-scoped state per instance. `TimelineBar.tsx` owns the controller via `useMemo([])` (one per dialog session) and passes `verificationState`, `hasGuestToken`, and `turnstileSession` down as props. The widget host element is created imperatively (never React-owned) and reparented across mountpoints via `ensureMounted(slot, siteKey)`, so the same widget instance and the same token survive cascade flips between Share and Trim guest surfaces inside one dialog session. The state machine is `idle | mounting | ready | preparing | load-failed | challenge-error`. Mount uses `appearance: 'interaction-only'` AND `execution: 'execute'` so mount cost is independent of solve cost; solving is deliberate via `warm()`. A surface-active flag drives proactive 4-minute refresh and expired-callback auto-rewarm. A 10-second `LOAD_TIMEOUT_MS` watchdog escalates a stuck mount to `'load-failed'`; a 15-second `EXECUTE_TIMEOUT_MS` watchdog escalates a stuck solve (no `callback` / `error-callback` / `expired-callback` from Cloudflare) to `'challenge-error'`. Error-callback retries are capped at `ERROR_RETRY_CAP = 3` consecutive failures, after which the runtime parks at `'challenge-error'`. Each `runExecute` mints a fresh `activeSolveEpoch` and the callbacks honor `activeSolveEpoch !== null` to drop late callbacks after a settle. **Every retry path remounts the widget for fresh callback identity** via `remountWidgetForFreshCallbacks` — site-key rotation (`ensureMounted` with a different key), `resetToken` (the user-initiated retry path), AND `error-callback`'s auto-retry branch all bump `renderEpoch`, remove the old widget, and re-render before the next attempt's `execute()`. This is the only reliable way to give the next attempt per-attempt callback identity, because Cloudflare binds the callback / error-callback / expired-callback closures at `turnstile.render()` time and reuses them across executes. Same-render retries cannot fully drop stale callbacks. Recovery from `'challenge-error'` is **user-initiated only** — the panel renders a "Try verification again" button (testid `transfer-guest-verify-retry`) that calls `resetToken()` then `warm()`; auto-warming on `'challenge-error'` is deliberately omitted from `TimelineBar`'s warm-effect to prevent a tight retry loop against a broken challenge runtime. The `GuestTurnstileController` interface and `guestTurnstileControllerRef` are removed.

**Rationale:** Separating mount from solve via `execution: 'execute'` is what makes one-widget-per-session viable — reparenting becomes free and the previously-issued token stays valid across surface flips, so the user does not re-solve a challenge they already passed. Imperative host ownership escapes React reconciliation (Turnstile's own DOM and React's diff were not friends), and the closure-scoped state (no module-level singletons) keeps test isolation and dispose semantics clean. The two watchdogs cover orthogonal failure modes: `LOAD_TIMEOUT_MS` for "widget never mounted" (script blocked, render stalled), `EXECUTE_TIMEOUT_MS` for "widget mounted but solve never settles" (Cloudflare's challenge runtime blocked by Trusted Types / inline-script CSP rules in its own iframe — observed in the field; without this watchdog the UI parks at `'preparing'` forever). `'challenge-error'` is recoverable but only via a user-deliberate action: the retry button gives the user a clear recovery affordance, while excluding `'challenge-error'` from auto-warm prevents a tight `warm → execute → timeout → challenge-error → warm → …` loop when the underlying challenge runtime is broken. The retry cap prevents the same loop in the error-callback path. Hoisting ownership to `TimelineBar` matches the dialog-session lifetime — the controller's lifetime is the dialog's, not any individual panel's.

**Alternatives rejected:** Keep lifecycle in `QuickShareDestinationPanel` and add a pinning ref to survive flips (the panel still unmounts; pinning a ref does not preserve the live widget's DOM); a module-level singleton (breaks test isolation, leaks state across dialog sessions, and entangles unrelated tabs); React-owned host element (loses to reconciliation when the parent re-renders mid-solve); uncapped error-callback retries (tight loops against a broken Turnstile endpoint); auto-warming on `'challenge-error'` (creates a tight retry loop with the solve watchdog); no solve watchdog (parks the UI in `'preparing'` indefinitely when Cloudflare's challenge runtime is blocked — the original failure documented in the bug report).

**Consequences:** One widget instance per dialog session, one token reused across both guest surfaces, no re-solve on cascade flip. `verificationState` is the single source of truth for CTA copy (D144 wires `'load-failed'` to "Verification unavailable"). A stuck solve cannot park the UI forever — `EXECUTE_TIMEOUT_MS` escalates to `'challenge-error'`. Recovery from `'challenge-error'` is the user clicking the panel's "Try verification again" button (testid `transfer-guest-verify-retry`), which calls `resetToken` (clearing in-flight watchdog + retry counter + widget) then `warm`. Tests are split: `tests/unit/turnstile-session.test.ts` covers the runtime in isolation (~33 tests including five for the solve watchdog); `tests/unit/turnstile-session-dialog-integration.test.tsx` covers reparenting, cascade-flip survival, and the retry-button source contract.

**Evidence:** `lab/js/runtime/turnstile-session.ts` (`createTurnstileSession`, `LOAD_TIMEOUT_MS`, `EXECUTE_TIMEOUT_MS`, `PROACTIVE_REFRESH_AT_MS`, `ERROR_RETRY_CAP`); `lab/js/components/timeline/TimelineBar.tsx` (`useMemo([])` controller, `verificationState` / `hasGuestToken` / `turnstileSession` prop wiring; warm-effect deliberately gated on `(ready|preparing)` only); `lab/js/components/timeline/timeline-transfer-dialog.tsx` (`QuickShareDestinationPanel` — defined inline in the dialog file — now consumes the controller via props, renders the empty `data-turnstile-mountpoint` slot, and renders the `transfer-guest-verify-retry` button when `verificationState === 'challenge-error'`; no longer owns lifecycle); tests `tests/unit/turnstile-session.test.ts`, `tests/unit/turnstile-session-dialog-integration.test.tsx`.

**Related ADRs:** D143 (`interaction-only` + `execution: 'execute'` mount profile); D144 (`'load-failed'` → "Verification unavailable" UI surface owned by this runtime); D139 (separate guest endpoint — the token this runtime issues is what `X-Turnstile-Token` carries to `/api/capsules/guest-publish`).
