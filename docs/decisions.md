...
**Decision:** Client-side Turnstile lifecycle moves out of `QuickShareDestinationPanel` into a session-scoped runtime: `lab/js/runtime/turnstile-session.ts` exposes `createTurnstileSession({ onStateChange, onTokenChange }, api?)` returning a `TurnstileSessionController` with closure-scoped state per instance. `TimelineBar.tsx` owns the controller via `useMemo([])` (one per dialog session) and passes `verificationState`, `hasGuestToken`, and `turnstileSession` down as props. The widget host element is created imperatively (never React-owned) and reparented across mountpoints via `ensureMounted(slot, siteKey)`, so the same widget instance and the same token survive cascade flips between Share and Trim guest surfaces inside one dialog session. The state machine is `idle | mounting | ready | preparing | load-failed | challenge-error`. Mount uses `appearance: 'interaction-only'` AND `execution: 'execute'` so mount cost is independent of solve cost; solving is deliberate via `warm()`. A surface-active flag drives proactive 4-minute refresh and expired-callback auto-rewarm. A 10-second load watchdog escalates to `'load-failed'`. Error-callback retries are capped at `ERROR_RETRY_CAP = 3` consecutive failures, after which the runtime parks at `'challenge-error'` and waits for a deliberate `warm()` from the host on re-entry. The `GuestTurnstileController` interface and `guestTurnstileControllerRef` are removed.

**Rationale:** Separating mount from solve via `execution: 'execute'` is what makes one-widget-per-session viable — reparenting becomes free and the previously-issued token stays valid across surface flips, so the user does not re-solve a challenge they already passed. Imperative host ownership escapes React reconciliation (Turnstile's own DOM and React's diff were not friends), and the closure-scoped state (no module-level singletons) keeps test isolation and dispose semantics clean. `'challenge-error'` is recoverable rather than terminal because Cloudflare's transient errors are common and the user has a clear deliberate action (re-engage the surface, which calls `warm()`); the retry cap prevents a failing Cloudflare endpoint from driving the runtime into a tight `reset → execute → error` retry loop. Hoisting ownership to `TimelineBar` matches the dialog-session lifetime — the controller's lifetime is the dialog's, not any individual panel's.

**Alternatives rejected:** Keep lifecycle in `QuickShareDestinationPanel` and add a pinning ref to survive flips (the panel still unmounts; pinning a ref does not preserve the live widget's DOM); a module-level singleton (breaks test isolation, leaks state across dialog sessions, and entangles unrelated tabs); React-owned host element (loses to reconciliation when the parent re-renders mid-solve); uncapped error-callback retries (tight loops against a broken Turnstile endpoint).

**Consequences:** One widget instance per dialog session, one token reused across both guest surfaces, no re-solve on cascade flip. `verificationState` is the single source of truth for CTA copy (D144 wires `'load-failed'` to "Verification unavailable"). Recovery from `'challenge-error'` is host-driven, so the destination panel's re-entry path must call `warm()` rather than recreating the widget. Tests are split: `tests/unit/turnstile-session.test.ts` covers the runtime in isolation (~27 tests); `tests/unit/turnstile-session-dialog-integration.test.tsx` covers reparenting and cascade-flip survival in the dialog.

**Evidence:** `lab/js/runtime/turnstile-session.ts` (`createTurnstileSession`, `LOAD_TIMEOUT_MS`, `PROACTIVE_REFRESH_AT_MS`, `ERROR_RETRY_CAP`); `lab/js/components/timeline/TimelineBar.tsx` (`useMemo([])` controller, `verificationState` / `hasGuestToken` / `turnstileSession` prop wiring); `lab/js/components/timeline/timeline-transfer-dialog.tsx` (`QuickShareDestinationPanel` — defined inline in the dialog file — now consumes the controller via props and renders only the empty `data-turnstile-mountpoint` slot; no longer owns lifecycle); tests `tests/unit/turnstile-session.test.ts`, `tests/unit/turnstile-session-dialog-integration.test.tsx`.
