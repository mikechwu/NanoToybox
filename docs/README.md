...
| [Viewer](viewer.md) | Trajectory viewer, Watch app controls, and Watch→Lab remix entry |
...
- **Watch→Lab remix** — Watch's bottom-chrome `WatchLabEntryControl` is a split capsule (Open Lab | Continue) whose Continue half snaps to the current frame and hands the capsule to a newly opened Lab tab via a normalized payload, `localStorage` handoff token, and `/lab/?from=watch&handoff=…` URL flag that suppresses Lab's default auto-load; Lab consumes the token and runs a transactional hydrate (authored atom colors, camera continuity, and motion/timeline provenance all carried across), and the hydrated scene itself is the arrival acknowledgement — the earlier discovery-hint runtime and provenance pill have been removed in favor of a CSS-only hover tooltip on the Continue half. See [architecture.md](architecture.md) for module boundaries + hydrate flow and [viewer.md](viewer.md) for the control, tooltip, click-ownership contract, and seed builder
