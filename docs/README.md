...
- **Watch→Lab remix** — Watch exposes a Lab-entry control that hands the current capsule off to Lab via a normalized payload + hydration lock; see [architecture.md](architecture.md) for module boundaries + hydrate flow and [viewer.md](viewer.md) for the control, hint, click-ownership contract, and seed builder
