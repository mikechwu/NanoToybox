...
- **lazy backfill** — on-read rebake performed by the poster route and `/api/account/capsules` (and backfill scripts) when a row's stored thumb rev is behind `CURRENT_THUMB_REV` or the stored scene is bondless (legacy atoms-only bake). The poster route heals synchronously so the returned poster carries bonds; the account API schedules the heal via `scheduleBackground` / `ctx.waitUntil` and serves current data on the next read. Shared implementation in `src/share/capsule-preview-heal.ts` (`rebakeSceneFromR2`, `healBondlessRow`, `sceneIsBondless`).
