...
export const CONFIG = {
  ...
  physics: {
    ...
    vHardMax: 0.15,         // Å/fs — per-atom velocity cap (sole post-integration emergency guard)
    dampingDefault: 0.0,     // velocity reduction factor per legacy batch (4 steps). Applied per-step internally as (1-d)^(1/4).
    // ── Force saturation caps (validated after zero-hit validation suite — see .reports/force-cap-telemetry-phase1.md) ──
    // fMaxInteraction: Δv_interaction_max ≈ 0.048 Å/fs per substep → F ≈ 120 eV/Å
    // fRepulsionStart: above stable-sim high tail; old global clamp was 50, atoms typically < 30
    // fMaxInternal: Δv_stable_limit ≈ 0.032 Å/fs → F ≈ 80 eV/Å
    fMaxInteraction: 120.0, // eV/Å — interaction-force asymptotic cap
    fRepulsionStart: 40.0,  // eV/Å — internal-force activation threshold (exact physics below this)
    fMaxInternal: 80.0,     // eV/Å — internal-force asymptotic cap (per-atom thresholded saturation)
