 ...
 */
import { wireSegmented } from '../shared/segmented';
import { requireEl } from '../shared/require-el';

export class DockController {
  _dockEl: HTMLElement;
  _addBtn: HTMLElement;
  _addIcon: HTMLElement;
  _addLabel: HTMLElement;
  _pauseBtn: HTMLButtonElement;
  _settingsBtn: HTMLButtonElement;
  _cancelBtn: HTMLElement;
  _onAdd: (() => void) | null;
  _onPause: (() => void) | null;
  _onSettings: (() => void) | null;
  _onCancel: (() => void) | null;
  _onModeChange: ((mode: string | undefined) => void) | null;
  _handlers: { add: () => void; pause: () => void; settings: () => void; cancel: () => void };
  _disposeModeSeg: (() => void);

  /**
   * @param {object} opts - DOM element refs
   */
  constructor({ dockEl, addBtn, addIcon, addLabel, modeSeg, pauseBtn, settingsBtn, cancelBtn }: {
    dockEl: HTMLElement; addBtn: HTMLElement; addIcon: HTMLElement; addLabel: HTMLElement;
    modeSeg: HTMLElement; pauseBtn: HTMLElement; settingsBtn: HTMLElement; cancelBtn: HTMLElement;
  }) {
    ...
    this._pauseBtn = requireEl('dock-pause', pauseBtn) as HTMLButtonElement;
    this._settingsBtn = requireEl('dock-settings', settingsBtn) as HTMLButtonElement;
  ...
  onAdd(cb: () => void) { this._onAdd = cb; }
  /** Register callback for Pause button click. */
  onPause(cb: () => void) { this._onPause = cb; }
  /** Register callback for Settings button click. */
  onSettings(cb: () => void) { this._onSettings = cb; }
  /** Register callback for Cancel button click (during placement). */
  onCancel(cb: () => void) { this._onCancel = cb; }
  /** Register callback for mode change (receives mode string). */
  onModeChange(cb: (mode: string | undefined) => void) { this._onModeChange = cb; }
